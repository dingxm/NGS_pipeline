...
gunzip $target/*R1*
gunzip $control/*R1*
bowtie -S -m 1 -p 4 $mm10_reference -q $target/*R1* --sam-RG ID:$target --sam-RG SM:$target | samtools view -bS - > ${target}_Bowtie_Alignment/${target}.bam
bowtie -S -m 1 -p 4 $mm10_reference -q $control/*R1* --sam-RG ID:$control --sam-RG SM:$control | samtools view -bS - > ${target}_Bowtie_Alignment/${control}.bam
...
bedtools genomecov -ibam ${target}.sort.bam -g /u/home/d/dingxm/mm10_chrom_size.txt -bg > ${target}.bedgraph
~/scripts/bdf2bw.sh ${target}.bedgraph /u/home/d/dingxm/mm10_chrom_size.txt
...
macs2 callpeak -t ${target}.sort.bam -c ${control}.sort.bam -n ${target} -g mm -p 0.00001 --outdir MACS_result
