...
bwa mem $reference -t 4 -M -R "$RG" $i/*R1* $i/*R2* | samtools view -bS - > BWA_Alignment/${i}.bam
#single read
...
#  
  samtools index BWA_Alignment/${i}.sorted.bam
##  samtools index BWA_Alignment/${i}.split.bam
  echo `date`
  java -Xmx8g -jar ~/GenomeAnalysisTK.jar -T RealignerTargetCreator -R $reference -known $gold_indel -L ~/hg19_coding_re.bed -ip 100 -o BWA_Alignment/${i}.sorted.bam.list -I BWA_Alignment/${i}.sorted.bam 
  echo `date`
  java -Xmx8g -jar ~/GenomeAnalysisTK.jar -T IndelRealigner -R $reference -known $gold_indel -I BWA_Alignment/${i}.sorted.bam -targetIntervals BWA_Alignment/${i}.sorted.bam.list -o BWA_Alignment/${i}.realigned.bam 
  echo `date`
...
then
  find *bam | parallel '/u/nobackup/lixuser/data/CAP_miRNA/bin/bedtools coverage -hist -abam {} -b ~/hg19_coding_merge.bed | grep ^all > {}.hist.all.txt'
