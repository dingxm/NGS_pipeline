...
length=$4
mkdir rsem_mm10
mkdir rsem_mm10lincRNA
ln -s /u/nobackup/lixuser/data/HiSeq2000_data/GenomeReference/Mus_musculus/UCSC/mm10/Sequence/Bowtie2Index/genome.* .
mouse_genes_gtf="/u/nobackup/lixuser/data/HiSeq2000_data/GenomeReference/Mus_musculus/UCSC/mm10/Annotation/Archives/archive-2014-05-23-16-05-10/Genes/genes.gtf"
cd $sampleid
...
if [ $reads == 2 ]; then
  ...
  tophat -p 8 -G  $mouse_genes_gtf -g 1 -o ${sampleid}_thout genome  ${sampleid}/*R1* ${sampleid}/*R2*
    ...
    rsem-calculate-expression --bowtie2 -p 4 --no-bam-output --fragment-length-mean 170.0 --fragment-length-sd 60.0 $sampleid/*R1*  /u/nobackup/lixuser/data/mm10_rsem_index/mm10_ref_rsem rsem_mm10/$sampleid
    ...
    tophat -p 4 -G $mouse_genes_gtf -g 1 -o ${sampleid}_thout genome ${sampleid}/*R1*
fi
cd ${sampleid}_thout
