...
mkdir rsem_tair10
mkdir rsem_tair10lincRNA

ln -s /u/project/lixuser/data/HiSeq2000_data/GenomeReference/Arabidopsis_thaliana/Ensembl/TAIR10/Sequence/Bowtie2Index/genome.* .
tair10_genes_gtf="/u/project/lixuser/data/HiSeq2000_data/GenomeReference/Arabidopsis_thaliana/Ensembl/TAIR10/Annotation/Archives/archive-2015-07-17-14-28-46/Genes/genes2.gtf"
ref_flat=/u/project/lixuser/data/HiSeq2000_data/GenomeReference/Arabidopsis_thaliana/Ensembl/TAIR10/Annotation/Archives/archive-2015-07-17-14-28-46/Genes/refFlat.txt
#mkdir rsem_hg19lincRNA
#Next500 
if [ $reads == 2 ]; then
    
    rsem-calculate-expression --bowtie2 -p 4 --no-bam-output --paired-end $sampleid/*R1* $sampleid/*R2* /u/project/lixuser/data/tair10_rsem_index/tair10_ref_rsem rsem_tair10/$sampleid
    ...
    tophat -p 4 -G $tair10_genes_gtf -g 1 -o ${sampleid}_thout genome ${sampleid}/*R1* ${sampleid}/*R2*
else

    rsem-calculate-expression --bowtie2 -p 4 --no-bam-output ${sampleid}/*R1*  --fragment-length-mean 180.0 --fragment-length-sd 70.0 /u/project/lixuser/data/tair10_rsem_index/tair10_ref_rsem rsem_tair10/$sampleid
    rsem-calculate-expression --bowtie2 -p 4 --no-bam-output ${sampleid}/*R1*  --fragment-length-mean 180.0 --fragment-length-sd 70.0 /u/project/lixuser/data/tair10lincRNA_rsem_index/tair10lincRNA_ref_rsem rsem_tair10lincRNA/$sampleid
    tophat -p 4 -G $tair10_genes_gtf -g 1 -o ${sampleid}_thout genome  ${sampleid}/${sampleid}*R1*
fi
# alignment QC
cd ${sampleid}_thout
